...
class TAIFEXFetcher:
    ...
    def fetch_tx_settlement(self) -> Dict[str, Any]:
        """
        抓取台指近月期貨結算價與漲跌
        Returns:
            {'value': 結算價, 'change_pct': 漲跌%}
        """
        try:
            url = f"{self.BASE_URL}/cht/3/futDailyMarketReport"
            params = {
                'queryType': '2',
                'marketCode': '0',
                'dateaddcnt': '',
            ...
            if tables:
                ...
                # 第一行即為近月合約
                row = df.iloc[0]
                settle = float(row.iloc[5])   # 最後成交價
                # 漲跌% 為字串如 '▲1.37%' 或 '▼0.52%'
                change_str = str(row.iloc[7]).replace('%', '').strip()
                change_str = change_str.replace('▲', '+').replace('▼', '-').replace('△', '+').replace('▽', '-')
                change_pct = round(float(change_str), 2) if change_str else None
                return {'value': settle, 'change_pct': change_pct}
        except Exception as e:
            print(f"[WARNING] 抓取台指近月期貨失敗: {e}")
...
class RiskMonitor:
    ...
    def fetch_all_data(self):
        ...
        # 4. 台指近月期貨
        tx_near = taifex_fetcher.fetch_tx_settlement()
            ...
            'indicators': [
                {
                    'category': '大盤',
                    'name': '加權指數 (TWII)',
                    'value': yf_data['taiex']['value'],
                    'change': yf_data['taiex']['change_pct'],
                    'unit': '',
                    'risk': self._assess_risk('taiex', yf_data['taiex']['change_pct'])
                },
                {
                    'category': '大盤',
                    'name': '櫃買指數 (OTC)',
                    'value': yf_data['otc']['value'],
                    'change': yf_data['otc']['change_pct'],
                    'unit': '',
                    'risk': self._assess_risk('otc', yf_data['otc']['change_pct'])
                },
                {
                    'category': '大盤',
                    'name': '台指近月期貨',
                    'value': tx_near['value'],
                    'change': tx_near['change_pct'],
                    'unit': '',
                    'risk': self._assess_risk('tx_near', tx_near['change_pct'])
                },
                {
                    'category': '總經',
                    'name': '美債10年殖利率',
